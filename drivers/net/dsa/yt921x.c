...
#define simple_strntoul(a, b, c, d) simple_strtoul(a, b, c)
#undef dev_dbg
...
yt921x_dsa_port_change_mtu(struct dsa_switch *ds, int port, int new_mtu)
{
	...
	pr_dbg("%s: port %d, mtu %d, frame size %d\n", __func__,
...
static int
yt921x_acl_rule_ext_parse_entries(struct yt921x_acl_rule_ext *ruleext,
				  u16 ports_mask,
				  const struct flow_cls_offload *cls)
{
	...
	for (unsigned int i = 0; i < size; i++)
		pr_dbg("ACL entry %d: %08x %08x %08x %08x\n", i, entries[i].key[0], entries[i].key[1], entries[i].mask[0], entries[i].mask[1]);
...
static int
yt921x_acl_rule_ext_parse_action(struct yt921x_acl_rule_ext *ruleext,
				 const struct flow_cls_offload *cls,
				 struct yt921x_priv *priv, int port)
{
	const struct flow_rule *rule = flow_cls_offload_flow_rule(cls);
	...
	flow_action_for_each(i, act, &rule->action)
		...
		case FLOW_ACTION_POLICE: {
			...
			res = yt921x_police_validate(police, &rule->action, act,
						     extack);
			if (res)
				return res;
			...
			action[0] |= YT921X_ACL_ACTa_METER_EN |
				     YT921X_ACL_ACTa_METER_ID(meterid);
			break;
		}
...
static int
yt921x_acl_rule_ext_parse(struct yt921x_acl_rule_ext *ruleext, int port,
			  ...
			  struct yt921x_priv *priv)
{
	...
	res = yt921x_acl_rule_ext_parse_action(ruleext, cls, priv, port);
	if (res)
		return res;
	res = yt921x_acl_rule_ext_parse_entries(ruleext, BIT(port), cls);
	if (res)
		return res;

	ruleext->r.cookie = cls->cookie;
	return 0;
}
		   ...
		   struct netlink_ext_ack *extack)
{
	...
	for (unsigned int i = YT921X_ACL_ENT_PER_BLK - entscnt + 1; i-- > 0;)
		if (candidates[i] >= 0)
			return YT921X_ACL_ENT_PER_BLK * candidates[i] +
			       ffz(priv->acl_masks[i]);

	/* TODO: rearrange existing rules */
...
static int yt921x_acl_del(struct yt921x_priv *priv, unsigned long cookie)
{
	struct device *dev = to_device(priv);
	       ...
	       struct netlink_ext_ack *extack)
{
	...
	if (ruleext->r.action[0] & YT921X_ACL_ACTa_METER_EN) {
		unsigned int meterid = FIELD_GET(YT921X_ACL_ACTa_METER_ID_M,
						 ruleext->r.action[0]);
			  ...
			  struct flow_cls_offload *cls, bool ingress)
{
	...
	res = yt921x_acl_rule_ext_parse(&ruleext, port, cls, ingress, priv);
	if (res)
		return res;
...
yt921x_dsa_port_lag_leave(struct dsa_switch *ds, int port, struct dsa_lag lag)
{
	...
	pr_dbg("%s: port %d, id %u\n", __func__, port, lag.id);
			 ...
			 struct netlink_ext_ack *extack)
{
	...
	pr_dbg("%s: port %d, vid %d, flags 0x%x\n", __func__, port, vid,
		vlan->flags);
			     ...
			     struct netlink_ext_ack *extack)
{
	...
	pr_dbg("%s: port %d, mask 0x%lx, flags 0x%lx\n", __func__, port,
...
yt921x_dsa_port_del_dscp_prio(struct dsa_switch *ds, int port, u8 dscp, u8 prio)
{
	...
	pr_dbg("%s: port %d, dscp %d, prio %d\n", __func__, port, dscp, prio);
...
static ssize_t
param_store(struct device *dev, struct device_attribute *attr,
	  const char *buf, size_t count)
{
	unsigned long v;
	char *e;

	v = simple_strntoul(buf, &e, 0, count);
	param = v;
	dev_err(dev, "Setting param to %lu\n", v);
